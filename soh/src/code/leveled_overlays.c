...
void ActorExperienceNumber_Draw(PlayState* play, Actor* actor) {
    ...
    // Color
    if (CVarGetInteger("LeveledAltAssets", 1)) {
...
void Leveled_Interface_DrawNextLevel(PlayState* play) {
    ...
    if (CVarGetInteger("LeveledAltAssets", 1)) {
        OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, (u8*)digitTextures[digit[i]], 8, 16,
                          posX - i * width + width * (digits - 1) + 50 - 1 + k, posY - 1 + j, 8, 16, 1024, 1024);
    } else {
        OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, (u8*)digitTextures[digit[i]], 8, 16,
                          posX - i * width + width * (digits - 1) + 38 - 1 + k, posY - 1 + j, 8, 16, 1024, 1024);
    }
            }
        }
        ...
        if (CVarGetInteger("LeveledAltAssets", 1)) {
            ...
            OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, (u8*)digitTextures[digit[i]], 8, 16,
                              posX - i * width + width * (digits - 1) + 50, posY, 8, 16, 1 << 10, 1 << 10);
        } else {
            gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 255, 255, 255, 255 - play->pauseCtx.alpha);
            OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, (u8*)digitTextures[digit[i]], 8, 16,
                              posX - i * width + width * (digits - 1) + 38, posY, 8, 16, 1 << 10, 1 << 10);
        }
    }
